...
$overview_video = get_field('overview_video') ?? null;
if( !empty( $overview_video ) ) {
	$video_url = get_field('video_url', $overview_video->ID) ?? null;
	$overview_thumb = get_field('video_thumbnail', $overview_video->ID) ?? null;
}
...
<section id="<?php echo esc_attr($id); ?>" class="module block <?php echo esc_attr($className); ?>">
	<?php if( !empty($overview_video) || !empty($videos) ):?>
		...
		<div class="tabs-content" data-tabs-content="<?=$block['id'];?>-tabs">
			<div class="tabs-panel overview is-active" id="overview-<?=$block['id'];?>" aria-hidden="false">
				...
				<button class="no-style relative" data-open="video-modal-<?=$block['id'];?>">
					<div class="img-play-wrap relative">
						<?php if( !empty( $overview_thumb ) ) {
							$imgID = $overview_thumb['ID'];
							$img_alt = trim( strip_tags( get_post_meta( $imgID, '_wp_attachment_image_alt', true ) ) );
							$img = wp_get_attachment_image( $imgID, 'full', false, [ "class" => "", "alt"=>$img_alt] );
							echo $img;
						}?>
						<div class="play-btn-wrap grid-x align-middle align-center">
							<svg xmlns="http://www.w3.org/2000/svg" width="145" height="145" viewBox="0 0 145 145"><g id="Group_347" data-name="Group 347" transform="translate(-913.442 -14487)"><circle id="Ellipse_1" data-name="Ellipse 1" cx="72.5" cy="72.5" r="72.5" transform="translate(913.442 14487)" fill="#383838" opacity="0.834"/><path id="Polygon_1" data-name="Polygon 1" d="M38,0,76,66H0Z" transform="translate(1024.442 14522) rotate(90)" fill="#ff1b59"/></g></svg>
						</div>
					</div>
				</button>
